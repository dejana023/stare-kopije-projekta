...
void Cpu::initializeGlobals(Image *im, bool dbl = false, int insi = 4) {
    ...
    double** _Pixels = _iimage->getPixels();
...
double Cpu::get_sum(double** I, int x1, int y1, int x2, int y2) {
    double z = I[y1 + 1][x1 + 1] + I[y2][x2] - I[y2][x1 + 1] - I[y1 + 1][x2];
    return z;
}

double Cpu::get_wavelet1(double** IPatch, int x, int y, double size) {
    double m = get_sum(IPatch, x + size, y, x - size, y - size) - get_sum(IPatch, x + size, y + size, x - size, y);
    return m;
}

double Cpu::get_wavelet2(double** IPatch, int x, int y, double size) {
    double n = get_sum(IPatch, x + size, y + size, x, y - size) - get_sum(IPatch, x, y + size, x - size, y - size);
    return n;
}
         ...
         // bool new_ch = 1;
          ...
          {     
              if(ready)
              {
                  ...
                  double m = get_wavelet1(_Pixels, r, c, step);
                  double n = get_wavelet2(_Pixels, r, c, step);
                  write_hard_double(addr_m, m);
                  write_hard_double(addr_n, n);

...
float Surf::readFromLookupFile(int index) {
    // Otvorite fajl lookup.txt za čitanje
    std::ifstream file("../data/lookup2.txt");
    
    // Proverite da li je fajl uspešno otvoren
    if (!file.is_open()) {
        // Fajl nije uspešno otvoren, tretirajte ovaj slučaj prema potrebi
        // Na primer, možete izbaciti grešku ili vratiti podrazumevanu vrednost
        throw std::runtime_error("Unable to open lookup.txt");
    }

    // Pomaknite se do željene pozicije u fajlu
    std::string line;
    for (int i = 0; i <= index; ++i) {
        if (!std::getline(file, line)) {
            // Ako dođemo do kraja fajla pre nego što stignemo do željene pozicije,
            // to može biti greška
            throw std::runtime_error("Index out of range in lookup.txt");
        }
    }
    ...
    // Konvertujemo pročitanu liniju iz fajla u float vrednost
...
void Surf::AddSample(num_i r, num_i c, num_f rpos,
 ...
 //cout << "step3: " << step << endl;
  weight = readFromLookupFile(static_cast<num_i>(rpos * rpos + cpos * cpos));
...
void Surf::createLookups(){
  ...
  for (int n=0;n<40;n++)
    _lookup2[n]=exp(-((double)(n+0.5))/8.0);
     ...
     std::ofstream outfile("../data/lookup2.txt");
  if (!outfile.is_open()) {
      std::cerr << "Nije moguće otvoriti fajl za pisanje" << std::endl;
      return;
  }

  // Ispisivanje vrednosti _lookup2 u fajl
  for (int i = 0; i < 40; ++i) {
      outfile << _lookup2[i] << std::endl;
  }

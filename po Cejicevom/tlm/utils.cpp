...
num_f toNum_f(unsigned char *buf)
{
    ...
    val += (uint64_t)(buf[0]) << 40;
    val += (uint64_t)(buf[1]) << 32;
    val += (uint64_t)(buf[2]) << 24;
    val += (uint64_t)(buf[3]) << 16;
    val += (uint64_t)(buf[4]) << 8;
    val += buf[5];

    return ((double)val / 1073741824.0); // 1073741824 = 2^30
}

void fromNum_f(num_f value, unsigned char *buf)
{
    uint64_t val = (uint64_t)(value * 1073741824.0); // 1073741824 = 2^30
...
/*void fromNum_f(num_f value, unsigned char *buf)
{
    int64_t scaled_value = (int64_t)(value * 1073741824.0); // 1073741824 = 2^30
    uint64_t val;

    if (scaled_value >= 0) {
        val = scaled_value;
    } else {
        val = (1ULL << 48) + scaled_value; // 1ULL << 48 = 2^48
    }
...
num_f toNum_f(unsigned char *buf)
{
    ...
    if (val >= (1ULL << 47)) { // 1ULL << 47 = 2^47
        scaled_value = val - (1ULL << 48); // 1ULL << 48 = 2^48
    } else {
        scaled_value = val;
    }

    return ((double)scaled_value / 1073741824.0); // 1073741824 = 2^30
}*/

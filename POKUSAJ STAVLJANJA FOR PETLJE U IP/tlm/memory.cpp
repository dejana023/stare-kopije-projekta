...
/*case TLM_READ_COMMAND:
        ...
        /*case tlm::TLM_READ_COMMAND:
			...
			for (unsigned int i = 0; i < len; i+=6)
			{
				...
				//sstd::cout << "adresa: " << (int)(addr-1) << " " << mem[addr-1] <<", citam: " << value << " " << (num_f)buf[i] << (num_f)buf[i+1] << std::endl << (num_f)buf[i+2] << std::endl << (num_f)buf[i+3] << std::endl << (num_f)buf[i+4] << std::endl << (num_f)buf[i+5] << std::endl;
			}
			pl.set_response_status(tlm::TLM_OK_RESPONSE);

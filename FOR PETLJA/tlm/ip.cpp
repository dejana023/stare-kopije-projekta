...
/*for (int i = 0; i < _width*_height; i++) {
    ...
    cout << "AddSample je pozvan " << callCount << " puta." << endl;*/
      ...
      if (rx > -1.0 && rx < (double) _IndexSize  &&
          ...
          num_i r = iy + i*step;
          num_i c = ix + j*step;
          num_i scale1 = (int)(scale);
          
        AddSample(r, c, rpos, cpos, rx, cx, scale1);
...
void Ip::AddSample(num_i r, num_i c, num_f rpos,
                     num_f cpos, num_f rx, num_f cx, num_i scale) {
    ...
    cout << "AddSample je pozvan " << callCount << " puta." << endl;*/
     if (r < 1+step  ||  r >= _height - 1-step  ||
      ...
      index1D = new num_f[_IndexSize * _IndexSize * 4];
                  ...
                  for (int i = 0; i < _IndexSize; i++)
                  {
                      for (int j = 0; j < _IndexSize; j++)
                      {
                          for (int k = 0; k < _IndexSize; k++) {
                             ...
                             // cout << index1D[index1D_index] << endl;
                          }
                      }
                  }
      ...
      /*            // Ispisivanje sadržaja vektora mem koristeći petlju
for (unsigned int i = 0; i < mem.size(); ++i) {
    std::cout << mem[i] << std::endl;
          ...
          //ISPIS PIXELSA    
                 ...
                 /* for (int x = 0; x < 64; ++x)
                  {
                      std::cout << static_cast<num_f>(index1D[64]) << " ";
                  }
                  std::cout << std::endl;*/
